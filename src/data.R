...
getWordID <- function(x, conn) {
  ...
  for (i in seq_along(x)) {
    q <- paste0("select id from dict where word == '", x[i], "'")
...
getAllNext <- function(x, conn) {
  ...
  q <- paste("select idnext, freq from", tabname, "where", where)
  dbGetQuery(conn, q)
}
...
getAllNext(c(810, NA, 160189), conn)
dbDisconnect(conn)
